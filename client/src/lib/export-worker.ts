...
/**
 * Total stamp cache cap for one worker rendering a whole sheet.
 *
 * Band mode runs several of these workers at once and divides this between
 * them, because the cap is a budget for the tab, not for the worker.
 */
...
class PngSink implements FilteredRowSink {
  ...
  constructor(outW: number, outH: number, exportDpi: number) {
    ...
    this.drained = (async () => {
      for (;;) {
        ...
        if (this.pendingBytes >= PNG_COALESCE_BYTES) this.fold();
      }
    })();
  }
  ...
  async abort(): Promise<void> {
    ...
    } catch { /* already broken; nothing to salvage either way */ }
  }
}
...
async function buildPngStreaming(input: ExportInput): Promise<Blob> {
  ...
  for (const strip of strips) {
    ...
    await writeStripRows(sink, imageData.data, stripH, filteredRowLen, rowBytes, bpp, prevRow);
    completedStrips++;
    self.postMessage({
      type: 'progress',
      requestId: input.requestId,
 ...
 * for the same rows, which is why `contextRow` exists: a row's filter may
 ...
 * transparent rows, so that a band and a serial strip covering the same rows
...
function filterBandToBuffer(
  ...
  const buf = new Uint8Array(rowCount * filteredRowLen);
  ...
  for (let r = 0; r < rowCount; r++) {
    ...
    const cur = pixels.subarray(rowStart, rowStart + rowBytes);
    filterRowAdaptive(cur, prevRow, bpp, rowBytes, buf, r * filteredRowLen);
    prevRow.set(cur);
  }
  return buf;
}

async function buildBand(input: BandInput): Promise<Uint8Array> {
  ...
  if (!stripHasContent(session.bounds, stripY, stripH)) {
    ...
    return new Uint8Array(stripH * (1 + rowBytes));
  }
...
self.onmessage = async function(e: MessageEvent) {
  ...
  if (e.data.type === 'band') {
    ...
    try {
      const filtered = await buildBand(input);
      self.postMessage(
        { type: 'band-result', requestId: input.requestId, index: input.index, filtered },
        [filtered.buffer],
      );
    } catch (err: any) {
