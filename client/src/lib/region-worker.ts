  ...
  for (let ci = 0; ci < colorCount; ci++) {
    ...
    for (let i = 0; i < totalPixels; i++) {
      ...
      while (head < queue.length) {
        ...
        // 4-connectivity — keeps diagonally-touching blobs as separate regions
    ...
    const regions = significant.map((comp, idx) => {
      for (const pi of comp.pixels) regionMap[pi] = idx;
      return {
        id: idx,
    ...
    // Assign orphan pixels to nearest significant region (bbox + margin check)
    const centroids = significant.map(comp => {
      let cx = 0, cy = 0;
      for (const pi of comp.pixels) { cx += pi % width; cy += (pi / width) | 0; }
      return { x: cx / comp.pixels.length, y: cy / comp.pixels.length };
    });
    const bboxMargin = Math.max(8, Math.sqrt(totalPixels) * 0.03);
    ...
    for (let i = 0; i < totalPixels; i++) {
      if (mask[i] === 1 && regionMap[i] === -1) {
        ...
        for (let r = 0; r < significant.length; r++) {
          ...
          const dx = px - centroids[r].x, dy = py - centroids[r].y;
