...
/**
 * Two-step offset smoothing to reduce nodes and create smoother cut paths.
 * 
 * For positive offsets (e.g., 0.25"): 
 *   First offset inward by -smoothingAmount, then offset outward by (desired + smoothingAmount)
 * 
 * For zero offset ("zero hero"):
 *   First offset outward by smoothingAmount, then offset inward by -smoothingAmount
 * 
 * This technique smooths the path and reduces nodes because the intermediate offset 
 * acts as a filter that removes small details and noise.
 * 
 * @param points - The path points (in pixels or inches, as long as consistent with smoothingAmount/offsetAmount)
 * @param offsetAmount - The desired final offset (can be 0 for "zero hero")
 * @param smoothingAmount - The intermediate offset amount (default 0.1" but pass in pixels if path is in pixels)
 */
export function smoothOffsetPath(points: Point[], offsetAmount: number, smoothingAmount: number): Point[] {
  if (points.length < 3) return points;
  
  console.log(`[smoothOffsetPath] Input: ${points.length} points, offset: ${offsetAmount}, smoothing: ${smoothingAmount}`);
  
  let intermediatePath: Point[];
  let finalPath: Point[];
  
  if (offsetAmount === 0) {
    // Zero hero: first expand outward, then shrink back inward
    // This smooths the original edge without changing the final position
    intermediatePath = offsetPathWithClipper(points, smoothingAmount);
    console.log(`[smoothOffsetPath] Zero hero - intermediate (expand +${smoothingAmount}): ${intermediatePath.length} points`);
    
    finalPath = offsetPathWithClipper(intermediatePath, -smoothingAmount);
    console.log(`[smoothOffsetPath] Zero hero - final (shrink -${smoothingAmount}): ${finalPath.length} points`);
  } else if (offsetAmount > 0) {
    // Positive offset: first shrink inward, then expand outward more
    // The inward shrink removes small details, then we expand to final size
    intermediatePath = offsetPathWithClipper(points, -smoothingAmount);
    console.log(`[smoothOffsetPath] Positive offset - intermediate (shrink -${smoothingAmount}): ${intermediatePath.length} points`);
    
    // If shrinking eliminated the path, fall back to direct offset
    if (intermediatePath.length < 3) {
      console.log(`[smoothOffsetPath] Intermediate path too small, falling back to direct offset`);
      return offsetPathWithClipper(points, offsetAmount);
    }
    
    finalPath = offsetPathWithClipper(intermediatePath, offsetAmount + smoothingAmount);
    console.log(`[smoothOffsetPath] Positive offset - final (expand +${offsetAmount + smoothingAmount}): ${finalPath.length} points`);
  } else {
    // Negative offset: first expand outward, then shrink inward more
    intermediatePath = offsetPathWithClipper(points, smoothingAmount);
    console.log(`[smoothOffsetPath] Negative offset - intermediate (expand +${smoothingAmount}): ${intermediatePath.length} points`);
    
    finalPath = offsetPathWithClipper(intermediatePath, offsetAmount - smoothingAmount);
    console.log(`[smoothOffsetPath] Negative offset - final (shrink ${offsetAmount - smoothingAmount}): ${finalPath.length} points`);
    
    // If shrinking eliminated the path, fall back to direct offset
    if (finalPath.length < 3) {
      console.log(`[smoothOffsetPath] Final path too small, falling back to direct offset`);
      return offsetPathWithClipper(points, offsetAmount);
    }
  }
  
  console.log(`[smoothOffsetPath] Node reduction: ${points.length} -> ${finalPath.length} (${Math.round((1 - finalPath.length / points.length) * 100)}% reduction)`);
