...
async function drawHighResImage(
  ...
  if (strokeSettings.enabled && strokeSettings.width > 0) {
    // Use the silhouette contour system for high-quality export
    const contourCanvas = createSilhouetteContour(image, strokeSettings);
    
    // Calculate scaling to fit the contour canvas within output canvas
    const contourAspectRatio = contourCanvas.width / contourCanvas.height;
    const canvasAspectRatio = canvasWidth / canvasHeight;
    
    let drawWidth, drawHeight, offsetX, offsetY;
    
    if (contourAspectRatio > canvasAspectRatio) {
      drawWidth = canvasWidth;
      drawHeight = canvasWidth / contourAspectRatio;
      offsetX = 0;
      offsetY = (canvasHeight - drawHeight) / 2;
    } else {
      drawHeight = canvasHeight;
      drawWidth = canvasHeight * contourAspectRatio;
      offsetX = (canvasWidth - drawWidth) / 2;
      offsetY = 0;
    }
    
    // Draw the combined contour + image canvas
    ctx.drawImage(contourCanvas, offsetX, offsetY, drawWidth, drawHeight);
  } else {
