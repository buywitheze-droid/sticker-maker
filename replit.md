...
- **Download System**: Multiple download modes (Standard, High-res, Vector Quality, CutContour), true vector export formats (PDF, EPS, SVG) with edge tracing, CutContour export with magenta spot color. PDF CutContour uses pathPoints (inch coordinates) converted to PDF points via ×72. When shapes mode detects a perfect geometric shape (circle/ellipse/rectangle), the PDF generates true Bezier curves (4-segment cubic Bezier for circles/ellipses using kappa=0.5522847498) instead of polygon approximations. Shape info (type, center, radii in inches) is passed from the worker through contourData.shapeInfo. For non-shape contours, uses line segments with minimal RDP simplification (epsilon=0.01 inches).
